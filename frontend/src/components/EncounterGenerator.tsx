...
export function EncounterGenerator({
    ...
    // ===== Everything below is regarding monster loading and filtering =====
    ...
    // ===== The encounter =====
    // === Created encounter/Encounter builder ===
    const [encounterBuilder, setEncounterBuilder] = useState<EncounterBuilder>({
        monsters: {}
    });

    const addMonsterToCreatedEncounter = (monsterName: string) => {
        setEncounterBuilder(prevState => ({
            ...prevState,
            monsters: {
                ...prevState.monsters,
                [monsterName]: (prevState.monsters[monsterName] || 0) + 1
            }
        }));
    }

    const removeMonsterFromCreatedEncounter = (monsterName: string) => {
        setEncounterBuilder(prevState => {
            const newCount = (encounterBuilder.monsters[monsterName] || 0) - 1;

            if (newCount <= 0) {
                // Remove the monster from the encounter entirely
                const {[monsterName]: _, ...rest} = prevState.monsters;
                return {
                    ...prevState,
                    monsters: rest
                };
            ...
            return {
                ...
                monsters: {
                    ...prevState.monsters,
                    [monsterName]: newCount
                }
            };
        });
    }

    const addCreatedEncounterToInitiativeTracker = async () => {
        const encounterMonsters = await encounterService.generateMonsterInitiative(encounterBuilder);
        const handle = initiativeSidebarRef?.current;
        if (!handle) {
            return;
        }
        handle.addEntries(encounterMonsters)
    }
        ...
        <div className="space-y-6">
            <Tabs defaultValue="create" className="w-full">
                ...
                <TabsContent value="create" className="space-y-2">
                    <div className="mb-4 text-center">Create</div>
                    <div className="grid grid-cols-2 gap-4">
                                ...
                                {filteredMonsters.map((monster) => (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => addMonsterToCreatedEncounter(monster.name)}
                        ...
                        <div className="space-y-2 pl-4">
                            Selected Monsters
                            <div className="space-y-1">
                                {Object.keys(encounterBuilder.monsters).length === 0 ? (
                                    <p className="mt-3 mb-3">No monsters added yet.</p>
                                ) : (
                                    Object.entries(encounterBuilder.monsters).map(([name, count]) => (
                                        <div className="flex items-center justify-between">
                                            <p>{count}x {name}</p>
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => removeMonsterFromCreatedEncounter(name)}
                                                className="px-2 py-1 text-xs"
                                            >
                                                Remove
                                            </Button>
                            ...
                            <div className="space-y-1">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => addCreatedEncounterToInitiativeTracker()}>
                                    Add to initiative Tracker
                                </Button>
                            </div>
                        </div>
